...
export default function PurchaseSlider({
    ...
    //     const total_amount = rows.reduce((sum, r) => sum + (Number(r.qty) || 0) * (Number(r.purchase_rate) || 0), 0);
    //     const discount = Number(form.discount_amount) || 0;
    //     const vat = Number(form.vat_amount) || 0;
    //     const net_amount = total_amount - discount + vat;
    //     const due_amount = 0;
    //     // const due_amount = net_amount - (Number(form.paid_amount) || 0);
    //     return { total_amount, net_amount, due_amount };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res?.response_code === 200 || res?.success) {
                toast.success(mode === 'edit' ? 'Purchase updated successfully!' : 'Purchase created successfully!');
                onSaved?.();
                onClose();
            } else {
