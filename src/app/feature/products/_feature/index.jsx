...
export default function ProductsFeature({ initialProducts = [], initialCategories = [], shops = [], userInfo = {}, brand = [], units = [] }) {
    ...
    const openCreateWithBarcode = (scannedCode) => {
        setEditData({ barcode: scannedCode });
        setSliderOpen(true);
    };
    ...
    useEffect(() => {
        if (sliderOpen) return; // Don't trigger if the slider is already open
        ...
        const handleKeyDown = async (e) => {
            ...
            if (e.key === 'Enter' && barcodeBufferRef.current.length > 0) {
                ...
                const foundProduct = initialProducts.find(p =>
                    String(p.barcode) === scannedCode ||
                    String(p.product_code) === scannedCode
                );

                if (foundProduct) {
                    await openEdit(foundProduct);
                } else {
                    openCreateWithBarcode(scannedCode);
                }
