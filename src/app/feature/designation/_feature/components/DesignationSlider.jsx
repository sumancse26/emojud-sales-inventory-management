...
import { saveDesignationAction } from '../action';

const EMPTY_FORM = { id: null, designation_name: '', display_code: '', status: 0, company_id: null, user_id: null };
...
export default function DesignationSlider({ isOpen, onClose, userInfo, onSaved, editData }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(() => {
            if (editData?.id) {
                setForm({
                    id: editData.id,
                    designation_name: editData.designation_name ?? '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await saveDesignationAction(payload);
            if (res?.response_code === 200 || res?.success) {
                toast.success(isEdit ? 'Designation updated successfully!' : 'Designation saved successfully!');
                onSaved?.();
                onClose();
            } else {
            ...
            >
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200/60 dark:border-slate-800/50 shrink-0">
                    <div className="flex items-center gap-3">
                        <span className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-violet-100 dark:bg-violet-500/10">
                            {isEdit ? (
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">
                                {isEdit ? `Editing: ${editData?.designation_name}` : 'Fill in the details to create a designation'}
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    <div>
                        <label className={LABEL_CLS} htmlFor="designation_name">
                            Designation Name <span className="text-red-500 normal-case font-normal">*</span>
                        </label>
                        <input
                            id="designation_name"
                    ...
                    <div className="rounded-xl border px-4 py-3 bg-violet-50 dark:bg-violet-500/5 border-violet-200/60 dark:border-violet-500/20">
                        <p className="text-xs leading-relaxed text-violet-700 dark:text-violet-400">
                            {isEdit
                                ? 'You are editing an existing designation. Changes will be saved immediately after confirmation.'
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        >
                            {saving ? 'Saving…' : isEdit ? 'Update Designation' : 'Save Designation'}
                        </button>
                    </div>
                </form>
            </div>
