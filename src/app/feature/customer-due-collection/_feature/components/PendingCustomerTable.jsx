...
export default function PendingCustomerTable({ customers = [], onCollectNow }) {
    return (
        <TableCard title={`Pending Customer Dues (${customers.length})`}>
            {!customers.length ? (
                <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-600">
                    ...
                    <p className="text-sm font-medium">No pending customer dues</p>
                    <p className="text-xs mt-0.5">All customer dues are settled right now.</p>
                </div>
            ) : (
                <div className="overflow-x-auto">
                    <table className="w-full text-sm min-w-150">
                        <thead className="bg-slate-50/80 dark:bg-slate-900/40 border-b border-slate-200/60 dark:border-slate-800/40">
                            <tr>
                                {['#', 'Customer', 'Contact', 'Due', 'Action'].map(h => (
                        ...
                        <tbody>
                            {customers.map((customer, index) => (
                                <tr key={customer.id ?? customer.customer_code ?? index} className={`border-b border-slate-100 dark:border-slate-800/30 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors ${index % 2 ? 'bg-slate-50/30 dark:bg-slate-900/10' : ''}`}>
                                    ...
                                    <td className={TD}>
                                        <div className="flex items-center gap-2.5">
                                            <span className="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-emerald-500/10 flex items-center justify-center shrink-0 text-sm font-bold text-emerald-700 dark:text-emerald-400">{customer.customer_name?.charAt(0)?.toUpperCase() ?? 'C'}</span>
                                            <div className="min-w-0">
                                                <p className="font-semibold text-slate-800 dark:text-slate-100 truncate">{customer.customer_name}</p>
                                                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-mono mt-0.5">{customer.customer_code ?? '—'}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className={`${TD} text-xs text-slate-500 dark:text-slate-400`}>
                                        <div className="space-y-0.5">
                                            <p>{customer.phone || '—'}</p>
                                            <p className="truncate">{customer.email || customer.address || '—'}</p>
                                        </div>
                                    </td>
                                    <td className={`${TD} tabular-nums`}>
                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 text-xs font-semibold border border-rose-100 dark:border-rose-500/20">
                                            ...
                                            {fmt(customer.previous_due)}
                                        </span>
                                    </td>
                                    <td className={TD}>
                                        <button
                                            onClick={() => onCollectNow?.(customer)}
