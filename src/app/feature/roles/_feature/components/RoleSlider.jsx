...
import { saveRoleAction } from '../action';

const EMPTY_FORM = { id: null, role_name: '', display_code: '', status: 0, company_id: null, user_id: null };
...
export default function RoleSlider({ isOpen, onClose, userInfo, onSaved, editData }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await saveRoleAction(payload);
            if (res?.response_code === 200 || res?.success) {
                toast.success(isEdit ? 'Role updated successfully!' : 'Role saved successfully!');
                onSaved?.();
                onClose();
            } else {
        ...
        <>
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">
                                {isEdit ? `Editing: ${editData?.role_name}` : 'Fill in the details to create a role'}
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                    <div>
                        <label className={LABEL_CLS} htmlFor="role_name">
                            Role Name <span className="text-red-500 normal-case font-normal">*</span>
                        </label>
                        <input
                            id="role_name"
                            name="role_name"
                    ...
                    <div className="rounded-xl border px-4 py-3 bg-emerald-50 dark:bg-emerald-500/5 border-emerald-200/60 dark:border-emerald-500/20">
                        <p className="text-xs leading-relaxed text-emerald-700 dark:text-emerald-400">
                            {isEdit
                                ? 'You are editing an existing role. Changes will be saved immediately after confirmation.'
